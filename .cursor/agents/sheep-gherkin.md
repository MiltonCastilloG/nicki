...
Run `.cursor/skills/story-maker/SKILL.md` and `.cursor/skills/story-maker/story-format.md`. Read the spec **only** at the spec path your prompt gives. Write the story **only** at the output path your prompt gives. Never invent the path. Never write `status.json`.
