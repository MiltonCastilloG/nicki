...
description: "Review post-execute worktree changes against spec, plan, execution handoff, and optional review guidance; write current-task/reviews/<slug>.yaml with approved status and review content. Use when the user runs /review-execution or asks to review implementation in a worktree."
model: inherit
readonly: false
is_background: false
---
...
Read and follow `.cursor/skills/review-execution/SKILL.md`, `.cursor/skills/review-execution/review-format.md`, `.cursor/skills/spec-maker/spec-format.md`, `.cursor/skills/execute-plan/plan-format.md`, `.cursor/skills/execute-plan/execution-format.md`, and `.cursor/skills/review-triage/review-guidance-format.md`.
...
| Tool | Allowed |
|------|---------|
| read | yes — worktree scope root, task context, spec, plan, execution handoff, review guidance, changed files, CONTRIBUTING.md |
| write | yes — **only** `current-task/reviews/*.yaml` under the worktree scope root |
| delete | no |
| shell | yes — scope root only; git diff, lint, test, verify per plan |
| grep / glob / semantic_search | yes — worktree scope root only |
| task | no — do not spawn subagents |
| web_search / web_fetch | no |
| mcp | no |
| ask_question | yes — when spec/plan missing or pass/fail is ambiguous |
...
3. **Plan** — `@current-task/plans/<slug>.yaml`, inline YAML, or path (auto-load from worktree when omitted).
...
2. Load task context, spec, plan, execution handoff, and optional review guidance when present; compare implementation against requirements and plan steps.
3. Discover changes via git diff; flag scope creep.
4. Run plan verify commands (or CONTRIBUTING defaults), independent of execution evidence.
...
- **Write** only to `current-task/reviews/<slug>.yaml` under the scope root — never edit `src/`, `app/`, config, specs, plans, `current-task/current-task-context.yaml`, or other application files.
