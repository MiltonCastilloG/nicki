...
3. **Task context** — auto-load `current-task/current-task-context.yaml`, spec, subtask list, and execution from `current-task/` when present.
...
2. Load the review plus current-task context, spec, subtask list, execution, and any relevant source context.
...
- Never modify application code, specs, subtask lists, executions, or reviews.
