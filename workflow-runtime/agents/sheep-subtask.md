...
Run `workflow-runtime/skills/subtask-maker/SKILL.md`. Write the checklist **only** at the output path your prompt gives. When the prompt carries **approved review fixes**, update that existing file (append `## Fix`; preserve completed lines) — do not regenerate from the spec. Never write `status.json`.
