...
| `review` | worktree path, task id, chat; diff plus whatever planning files exist | nothing, except an optional `## Fix` appended to the subtask file | no `artifact`; `completed_status`, `open_questions`, `summary` carrying the verdict |
